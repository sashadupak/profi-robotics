...
class Controller:
    ...
    def setup(self, i):
        xyz_offset = self.odometry.pose.pose.position
    ...
    def update(self, dt, t):
        ...
        if t1 <= self.tf * self.laps:
            ...
            velocity.linear.x = dot_xi*cos(rpy[2]) #+ 0.1*(v[0] - X[0])
            velocity.linear.y = dot_yi*sin(rpy[2]) #+ 0.1*(v[1] - X[1])
            velocity.angular.z = 1*(theta - rpy[2])
        else:
            #self.i += 1
